...
def main():
    ...
    if not app_dir:
        ...
        print("  2. 命令行参数: python 汉化.py \"D:\\你的安装路径\\Command Code\"")
        print("  3. 直接运行: python localize.py apply \"D:\\你的安装路径\\Command Code\"")
        sys.exit(1)
    ...
    try:
        ...
        if result.returncode != 0:
            print()
            print("[失败] 汉化未完成。可手动运行查看详细信息:")
            print(f'  python "{localize_script}" apply "{app_dir}"')
            sys.exit(1)
        
        print()
        print("[完成] 汉化已完成。请重启 Command Code 查看效果。")
        
    except Exception as e:
        print(f"[错误] 执行汉化时出错: {e}")
        sys.exit(1)
