...
# -*- coding: utf-8 -*-
"""
Command Code 汉化脚本（英文文件名版本）
一键将 Command Code 桌面应用汉化为简体中文
"""
...
def main():
    print("=" * 43)
    print("  Command Code Chinese Localization")
    print("=" * 43)
    print()
    ...
    if not app_dir:
        ...
        print("  2. Command line parameter: python localize_cn.py \"D:\\Your\\Path\\Command Code\"")
        print("  3. Direct run: python localize.py apply \"D:\\Your\\Path\\Command Code\"")
        sys.exit(1)
    ...
    try:
        result = subprocess.run([sys.executable, localize_script, 'apply', app_dir], 
        ...
        if result.returncode != 0:
            print()
            print("[Failed] Localization not completed. You can manually run for details:")
            print(f'  python "{localize_script}" apply "{app_dir}"')
            sys.exit(1)
        
        print()
        print("[Done] Localization completed. Please restart Command Code to see the effect.")
        
    except Exception as e:
        print(f"[Error] Error during localization: {e}")
        sys.exit(1)
